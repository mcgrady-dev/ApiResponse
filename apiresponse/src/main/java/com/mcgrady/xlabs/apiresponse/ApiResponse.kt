...
import com.mcgrady.xlabs.apiresponse.exceptions.NoContentException
...
sealed interface ApiResponse<out T> {
    ...
    companion object {
        ...
        internal fun <T> ApiResponse<T>.operate(): ApiResponse<T> = apply {
            val globalOperators = ApiResponseInitializer.responseOperators
            globalOperators.forEach { globalOperator ->
                if (globalOperator is ApiResponseOperator<*>) {
                    ...
                    val scope = ApiResponseInitializer.coroutineScope
                    scope.launch {
